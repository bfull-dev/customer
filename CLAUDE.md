...
### 2026-07-09
- 休業日カスタマーサポート自動返信を追加（`4468b75`）
  - 入口は `sendMessage()`。お客様メッセージ受信時に有効判定し Re:Lation メールで自動返信
  - **グローバル1箇所管理**：App786 に設定レコード（`管理番号 = AUTO_REPLY_CONFIG`／レコードID 196）を1件用意。
    そのレコードの `自動返信`（チェック）／`開始日時_自動返信`／`解除日時_自動返信`／`自動返信メッセージ` だけを操作する
  - 有効判定：`isAutoReplyActive()` = チェックON **or** 開始日時≦現在≦解除日時（解除空＝上限なし）。cron 不使用・送信時判定
  - 3時間スロットリング：各お客様レコードの `最終通知送信日時`（既存の空きDATETIMEを流用）で管理。本文保存の同一 PUT に記録
  - **メール送信のみ**：`メッセージ履歴`／`最終送信者区分`／`担当者メッセージ` は変更せず、担当者パネルの「お客様が未対応」状態を保持
  - Discord 通知・App125同期・`対応完了`→`再問合せ` は従来どおり継続
  - 設定レコードはトークン無し・非公開のためポータル・cron 非対象。通常問合せで `AUTO_REPLY_CONFIG` を使わないこと
  - 環境変数 `AUTO_REPLY_CONFIG_KANRI` で設定レコードの管理番号キーを上書き可能（未設定時 `AUTO_REPLY_CONFIG`）
- 解除日時経過後の日時フィールド自動クリア cron を追加（`1a81e87`）
  - 毎時5分（`5 * * * *`）に設定レコードを確認し、`解除日時_自動返信` を過ぎていたら
    `開始日時_自動返信`／`解除日時_自動返信` を空欄に戻す（`自動返信` チェックは変更しない）
  - `getAutoReplyConfig()` を再利用し JS で期限判定（空 DATETIME クエリ全件一致問題を回避）。冪等
  - 機能上の自動解除は送信時判定で即時。この cron は日時フィールドの後片付けのみ（最大約1時間ラグ）
